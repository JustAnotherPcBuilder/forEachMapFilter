...
objects, a key, and a value and returns the array passed to the function with the new key and value added for each object 
...
object with the keys as the vowel and the values as the number of times the vowel appears in the string. 
...
function vowelCount(str){
    ...
    for(let elem of arr){
        if (!(elem in obj)){    
                        //returns the number of times elem is in arr
            obj[elem] = arr.filter((v) => (v === elem)).length;
        }
    }
    return obj;
...
function find(arr, searchValue) {
    let newarr = arr.filter((val) => val == searchValue);
    if(newarr){
        return newarr[0];
    }
...
function removeVowels(str) {
    return str.toLowerCase().split('').filter((char) => 'aeoiu'.indexOf(char) == -1).join('');
}
